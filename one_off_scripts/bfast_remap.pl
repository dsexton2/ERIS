...
#First we will need to merged *_1.filt.fastq and *_2.filt.fastq files into a single *.filt.fastq file.  A Ruby script exists to do this if you 	want to use it.  /stornext/snfs5/next-gen/solid/utilities/helpers/split_TGP.rb
...
sub msub_with_dependencies {
	...
	#my $cmd = "ruby /stornext/snfs5/next-gen/solid/utilities/helpers/split_TGP.rb $fastq_file_base";
	#my $dependency_id = &submit_job($cmd, "", $fastq_file_base);
	...
	$dependency_id = &submit_job($cmd, $dependency_id, $fastq_file_base);
...
sub submit_job {
	(my $command, my $dependency_id, my $fastq_file_base) = @_;
	#  moab_resources: "nodes=1:ppn=8,mem=28000mb"
	my $scheduler = Concordance::Common::Scheduler->new;
	$scheduler->command($command);
	$scheduler->job_name_prefix($fastq_file_base."_".$$."_".int(rand(5000))."bfast_remap.job");
	$scheduler->cores(8);
	$scheduler->memory(28000);
	$scheduler->priority("normal");
	$scheduler->dependency_list($dependency_id, $fastq_file_base);
	$scheduler->execute;
